...
async function fetchBTCCandles(): Promise<CandleData[]> {
  const response = await fetch("https://data.api.drift.trade/market/BTC-PERP/candles/1?limit=500");
...
function generateMomentumSignal(fastEMA: number[], slowEMA: number[], volumeFilter: boolean[]): number[] {
...
async function generateSignal(): Promise<Signal> {
  ...
  for (let s = 0; s < 5; s++) {
    ...
    const momentumSignals = generateMomentumSignal(fastEMA, slowEMA, volumeFilter);
